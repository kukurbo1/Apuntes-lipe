...
\usepackage{stmaryrd} %% cotniene triangleleqslant para subgrupos normales.
\usepackage{quiver}
\usepackage{mathtools}
\usepackage{mathrsfs} %% contiene mathscr.
...
\begin{example}
  $\SL_n(\R) = \{A \in \GL_n(\R) : \det(A) = 1\}$ es un subgrupo de $\GL_n(\R)$.
\end{example}
...
\begin{example}
  $S^1 = \{z \in \C : |z| = 1\}$ con la multiplicación.
...
\begin{definition}
  ...
  \[\gen{S} = \{g \in G : \exists n \geq 0, \quad g = s_1,\dots,s_n, \quad s_1,\dots,s_n \in S \cup S^{-1}\}.\]
...
\begin{definition}
  ...
  \[A_n = \ker(\phi) = \{\sigma \in S_n : \sgn(\sigma) = 1\}.\]
\end{definition}
...
\begin{definition}
  Si $H \leqslant G$, el \wea{índice de $H$ en $G$} es $$[G : H] = \abs{\frac{G}{H}}.$$
\end{definition}
...
\begin{definition}
  ...
  \begin{itemize}
    \item La \wea{órbita de $x \in X$} es $$\orb_G(x) = \{g \c x : g \in G\}.$$
    \item El \wea{estabilizador de $x \in X$} es $$\stab_G(x) = \{g \in G : g \c x = x\}.$$
  \end{itemize}
\end{definition}
...
\begin{definition}
  Decimos que una acción $G \acts X$ es \wea{fiel} si $$\forall g \in G \setminus \{1_G\}, \exists x \in X, \quad g\c x \neq x.$$
\end{definition}
...
\begin{example}
  Considere $S^1 = \{z \in \C : \abs z = 1\}, \ \alpha \in [0,1)$. Tenemos una acción $\Z \acts S^1$ dada por $$n \c z = z e^{2\pi in\alpha}.$$
...
\begin{definition}
  $G \acts X$ se dice \wea{transitiva} si $\forall x,y \in X, \exists g \in G : g\c x = y$.
\end{definition}
...
\end{theorem}
\begin{proof}
  Consideremos $$X = \{(x_1,\dots,x_p) \in G^p : x_1 x_2 \cdots x_p = 1_G\}.$$
  Notemos que $$X = \{(x_1,\dots,x_{p-1}, x_{p-1}^{-1}\cdots x_2^{-1}x_1^{-1}) : x_1,\dots,x_{p-1} \in G\}$$ y entonces $\abs X = \abs G^{p-1}$.
  ...
  \end{align*}
  Luego
  \[\abs{X} = \abs{G}^{p-1} = \sum_{x_i \in X/(\Z/p\Z)} |\orb_{\Z/p\Z} (x_i)|.\]
  Supongamos por contradicción que $\forall g \in G \setminus \{1_G\}, \ g^p \neq 1_G$. \\
...
\begin{example}
  ...
  \begin{itemize}
    ...
    \begin{lemma}
      Sea $G$ grupo finito y $G \acts X$ transitiva. Sea $N \normal X$ con $[G : N] = 2$ tal que $N \acts X$ no es transitiva.
...
Recordemos que
\[S_\oo = \Sym_0(\Z) = \{\sigma \in \Sym(\Z) : \abs{\sop(\sigma)} < \oo\}\]
donde $$\sop(\sigma) = \{n \in \Z : \sigma(n) \neq n\}.$$
...
\begin{definition}
  ...
  \[\Cay(G,S) = (G,\{(g,gs) : s \in S , g \in G\}).\]
\end{definition}

\begin{example}
  $G = \Z$, $S=\{1\}$, tenemos $E = \{(n,n+1) : n \in \Z\}$.
  \[\begin{tikzcd}
...
\begin{obs}
  ...
  donde $B_S(g,r) = \{h \in G : d_S(g,b) \leq r\}$.
...
\begin{definition}
  ...
  \[\abs{g}_S = \min\{n \in \N : g = s_1,\dots,s_n, \quad s_i \in S\}.\]
...
\begin{notation}
  ...
  \[B_S(g,r) \coloneqq \{h \in G : d_S(g,h) \leq r\}.\]
...
\begin{example}
  ...
  \[S = \set{\sum_{i=1}^d a_ie_i : a_i \in \{-1,0,1\}}\]
...
\begin{exercise}
  ...
  \[H = \set{\matr{1 & x & z \\ 0 & 1 & y \\ 0 & 0 & 1} \in \GL_3(\Z) : x,y,z \in \Z} \leqslant \GL_3(\Z).\]
...
\begin{theorem}
  [Gromov]
  Un grupo $G$ finitamente generado tiene crecimiento polinomial si t solo si $\exists H \leqslant G$ con $[G : H] < \oo$ tal que $H$ es nilpotente.
\end{theorem}
\begin{proof}
...
\begin{example}
  Si $X = \{a\}$, entonces
  \begin{align*}
    F(\{a\}) &= \{\w \in \{a,a^{-1}\}^\star : aa^{-1}, a^{-1} a \text{ no están en } \w\} \\
    &= \{a^n : n \geq 1\} \cup \{a^{-n} : n \geq 1\} \cup \{\e\} \\
    \simeq \Z.
  \end{align*}
\end{example}
...
\begin{definition}
  ...
  \[\mathrm{WP}_S(G) = \{\w \in (S \cup S^{-1})^\star : \underline{\w}_G = 1_G\}.\]
\end{definition}
...
\begin{example}
  ...
  Consideremos
  \begin{align*}
    X_1 &= \{[x:y] \in \R P^1 : \abs x > \abs y\}, \\
    X_2 &= \{[x:y] \in \R P^1 : \abs y > \abs x\}.
  \end{align*}
...
\begin{obs}
  ...
  \[\exp(G) = \lcm\{\ord(g) : g \in G\}.\]
\end{obs}
\begin{proof}
  Sea $M = \lcm\{\ord(g) : g \in G\}$. Notemos que $M \leq \exp(G)$, pues $\forall g \in G$, $\ord(g) \mid \exp(G)$.
...
\begin{example}
  Consideremos
  \[\H = \set{\matr{1 & a & c \\ 0 & 1 & b \\ 0 & 0 & 1} : a,b,c \in \Z}\]
  ...
  \begin{align*}
    \Z^2 &\simeq \set{\matr{1&a&b \\ 0 & 1 & 0 \\ 0 & 0 & 1} : a,b \in \Z} = H, \\
    \Z &\simeq \set{\matr{1&0&0 \\ 0&1&c \\ 0&0&1} : c \in \Z} = K.
  \end{align*}
  \begin{itemize}
    \item $\H = HK$ %% Comprobar
...
\subsection{Automorfismos de \texorpdfstring{$\zmz n$}{Z/nZ}}

\begin{obs}
  Notemos que $\zmz0 \simeq \zmz2$, por lo que basta analizar $n \geq 1$.
\end{obs}

\begin{definition}
  Durante esta sección, $(p_n)_n$ será al sucesión de números primos, ordenados de menor a mayor.
\end{definition}

\begin{obs}
  $nm = \gcd(m,n) \c \lcm(m,n)$.
\end{obs}

\begin{lemma}[Bezout]
  Si $a,b \geq 1$ son enteros, entonces existen $x,y \in \Z$ tales que $ax+by = \gcd(a,b)$.
\end{lemma}

\begin{definition}
  Sea $n \geq 2$ entero. 
  \[ \br{\znz n}^* = \{k \in \znz n \colon \gcd(k,n) = 1\}. \]
\end{definition}

\begin{proposition}
  $(\zmz n)^*$ es grupo con la multiplicación módulo $n$.
\end{proposition}

\begin{example}
  $((\zmz8)^*, \c) \simeq (\zmz2 \times \zmz2, +)$.
\end{example}

\begin{obs}
  Si $p \geq 2$ es primo, entonces 
  \[ \br{\znz p}^* = \{1,\dots, p-1\}. \]
\end{obs}

\begin{theorem}
  Sea $n \geq 2$, entonces 
  \[ \Aut\br{\znz n} \simeq \br{\znz n}^*. \]
\end{theorem}
\begin{proof}
  Sea $k \in \Z$, definimos $m_k \colon \zmz n \to \zmz n$ dada por $m_k(a) = ak \mod n$.
  Notemos que $m_k$ es endomorfismo. Además, $\gcd(k,n) = 1 \iff m_k$ es automorfismo.
  Sea $k$ coprimo con $n$, basta verificar que $\ker(m_k) = \{0\}$. 
  \[ \ker(m_k) = \set{a \in \znz n \colon ak = 0 \mod n}. \]
  Notemos que como $\gcd(k,n) = 1$, entonces existe $\ell \in \Z$ tal que $$k \c \ell = 1 \mod n.$$
  Sea $a \in \znz n$ tal que $ak = 0 \mod n$, entonces
  \[ 0 = ak \ell = a \mod n. \]
  Si $\gcd(k,n) > 1$, entonces 
  \[ \underbrace{\frac{n}{\gcd(n,k)}}_{\in \zmz n} \c k = n \c \underbrace{\frac{k}{\gcd(n,k)}}_{\in \Z} = 0 \mod n \]
  y luego $n/\gcd(k,n) \in \ker(m_k)$.

  Ahora, definimos la función 
  \begin{align*}
      M \colon \br{\znz n}^* &\to \Aut\br{\znz n} \\ 
      k &\mapsto m_k
  \end{align*}
  Esta función está definida. Es homomorfismo: 
  \[ M(k \c k') = m_{k \c k'} = m_k \circ m_{k'} = M(k) \circ M(k'). \]
  Es inyectiva: Si $k >1$, entonces $m_k(1) = k \neq 1$. Luego 
  \[ \ker(M) = \set{k \in \znz n \colon m_k = \id} = \{1\}. \]
  Es sobreyectiva, ya que si $\a \in \Aut(\zmz n)$ y $\ell \in \zmz n$, entonces 
  \[ \a(\ell) = \underbrace{\a(1) + \dots + \a(1)}_{\ell \ veces} = \ell \c \a(1). \]
  Luego $\forall \ell \in \zmz n$, $\a(\ell) = \ell \c \a(1)$, y entonces $\a = m_{\a(1)}$. Como $\a \in \Aut(\zmz n)$, entonces $\a(1) \in (\zmz n)^*$. Por lo tanto $\a = M(\a(1))$.
...
\begin{definition}
  Un conjunto $A$ dotado de dos operaciones $+$ y $\c$ se llama \wea{anillo} si
  \begin{enumerate}[(1)]
    \item $(A,+)$ es grupo abeliano (con neutro $0_A$),
    \item $(A,\c)$ es semigrupo (magma asociativo),
    \item $\c$ distribuye sobre $+$: $\forall a,b,c \in A$, 
    \[ a\c (b+c) = ab + ac, \quad (b+c) \c a = ba + ca. \]
  \end{enumerate}
\end{definition}

\begin{definition}
  Sea $(A,+,\c)$ un anillo. Decimos que es \wea{anillo con unidad} si $(A,\c)$ es monoide, es decir, $\exists 1_A \in A$ tal que $\forall a \in A$, $a \c 1_A = 1_A a = a$.
\end{definition}

\begin{obs}
  En general consideraremos anillos con unidad. A menos que se diga lo contrario, todos los anillos tendrán unidad.
\end{obs}

\begin{definition}
  Sea $(A,+,\c)$ anillo. Decimos que es \wea{conmutativo} si $\forall a,b \in A$, $ab = ba$.
\end{definition}

\begin{definition}
  $A$ es un \wea{anillo de división} si tiene unidad y $\forall a \in A \setminus \{0\}$ se cumple que $\exists a^{-1} \in A$ tal que $a a^{-1} = a^{-1} a = 1$.
\end{definition}

\begin{definition}
  $A$ es \wea{cuerpo} si es un anillo conmutativo y de división.
\end{definition}

\begin{definition}
  Dado un anillo $(A,+,\c)$ con unidad, decimos que $a \in A \setminus \{0\}$ es una \wea{unidad} si $\exists a^{-1} \in A$ tal que $aa^{-1} = a^{-1} a = 1$. Escribimos 
  \[ A^\times = \{a \in A \setminus \{0\}+ \colon a \text{ es unidad}\}. \]
\end{definition}

\begin{definition}
  Decimos que $a \in A$ es \wea{divisor de cero a la izquierda} si $\exists b \setminus \{0\}$ tal que $ab = 0$.
\end{definition}

\begin{definition}
  Decimos que $A$ es un \wea{dominio de integridad} si es un anillo conmutativo sin divisores de cero.
\end{definition}

\begin{obs}
  Sea $(A,+,\c)$.
  \begin{enumerate}[(1)]
    \item $\forall a \in A, \quad a \c 0 = 0 \c a = 0$.
    \item Si $A$ tiene unidad 1 y $1=0$, entonces $\abs A = 1$.
  \end{enumerate}
\end{obs}

\begin{example}
  $(\Z,+,\c)$ es un anillo conmutativo con unidad. No es anillo de división, ya que $\Z^\times = \{\pm 1\}$.
\end{example}

\begin{example}
  $(\Q,+,\c)$, $(\R,+,\c)$ y $(\C,+,\c)$ son cuerpos.
\end{example}

\begin{example}
  Sea $\xi \in \C$, definimos 
  \[ \Q[\xi] = \{a + b \xi \in \C : a,b \in \Q\}. \]
  Esto es un anillo.
\end{example}

\begin{exercise}
  Muestre que $\Q(\xi)$ es cuerpo.
\end{exercise}

\begin{example}
  $\Z(i) = \{a+bi \in \C \colon a,b \in \Z\}$.
\end{example}

\begin{example}
  Si $X$ es un conjunto, entonces $\R^X = \{f \colon X \to \R\}$ es un anillo con la suma punto a punto y multiplicación punto a punto.
\end{example}

\begin{example}
  Sea $\ds A = \{(a_n)_{n \geq 0} \in \R^\N \colon \lim_{n\to\oo} a_n = 0\}$ con la $+$ y $\c$ coordenada a coordenada. Este es un anillo sin unidad.
\end{example}

\begin{example}
  $M_n(\R)$ el conjunto de matrices $n \times n$ con coeficientes en $\R$. Este es un anillo no conmutativo.
\end{example}

\begin{example}
  $\R[x]$ el conjunto de polinomios con coeficientes en $\R$.
\end{example}

\begin{example}
  $(\zmz n,+,\c)$ es anillo conmutativo. Si $n$ es primo, entonces este es un cuerpo.
\end{example}

\fecha{Clase 16. Miércoles 29 de Abril}

\begin{definition}
  Sea $(R,+,\c)$ un anillo. El \wea{anillo de polinomios con coeficientes en $R$} es 
  \[ R[x] = \{(a_n)_{n \geq 0} \in R^\N \colon \exists k \geq 0, \forall \ell \geq k, a_\ell = 0_R\}. \]
  Definimos las siguientes operaciones en $R[x]$: Si $a = (a_n)_n$ y $b = (b_n)_n$
  \[ a + b = (a_n + b_n)_{n \geq 0}, \quad (a \c b)_n = \sum_{k \leq n} a_k b_{n-k}. \]
\end{definition}
...
\begin{obs}
  Existe una función evaluación natural $\mathrm{ev} \colon R[x] \times R \to R$ 
  \[ \mathrm{ev}(p,r) = \sum_{n \geq 0} (p_n) r^n. \]
\end{obs}

\begin{notation}
  Escribimos $(a_n)_{n \geq 0} = \sum_{n \geq 0} a_n x^n$.
\end{notation}

\begin{definition}
  Sea $p \in R[x]$ con $p \neq 0$, el \wea{grado de $p$} es 
  \[ \gr(p) = \min\{k \geq 0 \colon \forall \ell \geq k, p_\ell = 0\}. \]
\end{definition}

\begin{definition}
  Sea $(R,+,\c)$ un anillo. El \wea{anillo de matrices de $n \times n$ con coeficientes en $R$} es
  $$M_n(R) = \{(a_{ij})_{1 \leq i,j \leq n} \colon a_{i,j} \in R\}.$$
  Definimos las operaciones 
  \[ (A + B)_{ij} = A_{ij} + B_{ij}, \quad (A \c B)_{ij} = \sum_{k=1}^n A_{ik} \c B_{kj}. \]
\end{definition}

\begin{proposition}
  $(M_n(R),+,\c)$ es un anillo.
\end{proposition}

\begin{definition}
  Sea $(R,+,\c)$ un anillo. Diremos que $A \subset R$ es \wea{subanillo} si $(A,+, \c)$ es un anillo con las operaciones restringidas a $A \times A$.
\end{definition}

\begin{definition}
  Sea $(R,+,\c)$ un anillo y $S \subset R$. El \wea{anillo generado por $S$ en $R$} es 
  \[ \gen{S}_R = \gen{S} = \bigcap_{\substack{A \subset R \\ A \text{ subanillo} \\ S \subset A}} A. \]
\end{definition}

\begin{proposition}
  $\gen{S}$ es un subanillo de $R$.
\end{proposition}

\begin{example}
  Consideremos el cuerpo $(\C,+,\c)$ y $S = \{1,i\}$, entonces $\gen{S} = \Z[i]$.
\end{example}

\begin{definition}
  Sean $(A,+,\c)$ y $(B,+,\c)$ anillos. Una función $\vphi \colon A \to B$ se dice \wea{homomorfismo de anillos} si $\forall a,a' \in A$, 
  \[ \vphi(a+a') = \vphi(a) + \vphi(a') \quad\land\quad \vphi(aa') = \vphi(a) \vphi(a'). \]
\end{definition}

\begin{exercise}
  Sea $\vphi \colon A \to B$ homomorfismo de anillos. Muestre que $(\im(\vphi),+,\c)$ es subanillo de $B$.
\end{exercise}
\begin{proof}
  Como $\vphi \colon A \to B$ es homomorfismo de anillos, entonces $\vphi$ es homomorfismo de grupos entre $(A,+)$ y $(B,+)$. Luego $(\im(\vphi),+) \leqslant (B,+)$. Sean $b = \vphi(a)$ y $b' = \vphi(a')$, entonces 
  \[ bb' = \vphi(a) \vphi(a') = \vphi(aa') \in \im(\vphi). \]
  También el producto es claramente asociativo. Veamos distributividad: 
  \begin{align*}
      \vphi(a)(\vphi(b) + \vphi(c))
      &= \vphi(a)\vphi(b+c) \\
      &= \vphi(a(b+c)) \\
      &= \vphi(ab + ac) \\
      &= \vphi(ab) + \vphi(ac) \\
      &= \vphi(a)\vphi(b) + \vphi(a) \vphi(c).
  \end{align*}
\end{proof}

\begin{obs}
  De la definición de homomorfismo \wea{no} se concluye que $\vphi(1_A) = 1_B$. 
  \[ \vphi(1_A) = \vphi(1_A \c 1_A) = \vphi(1_A) \vphi(1_A) = \vphi(1_A)^2. \]
\end{obs}

\begin{example}
  Sea $R = (\Z,+,\c)$ y consideremos el anillo $R^2 = R \times R$. Sea $\vphi \colon R \to R^2$ dado por 
  \[ \vphi(a) = (a,0) \]
  tenemos que $\vphi$ es homomorfismo de anillos, pero $\vphi(1_R) \neq 1_{R^2}$.
  Ojo, $\vphi(1_R)$ es unidad en $\vphi(R)$, pero no es la unidad de $R^2$.
\end{example}

\begin{definition}
  Dado $\vphi \colon A \to B$ homomorfismo de anillos. El \wea{kernel de $\vphi$} es 
  \[ \ker(\vphi) = \{x \in A \colon \vphi(x) = 0_B\}. \]
\end{definition}

\begin{obs}
  $(\ker(\vphi),+_A) \normal (A,+)$.
  Además, si $a,b \in \ker(\vphi)$: 
  \[ \vphi(ab) = \vphi(a) \vphi(b) = 0 \ 0 = 0 \]
  \[ \implies ab \in \ker(\vphi). \]
  Por lo tanto, $(\ker(\vphi),+,\c)$ es un subanillo de $A$.

  Notemos que si $a \in \ker(\vphi)$ y $b \in A$, entonces 
  \begin{align*}
      \vphi(ab) &= \vphi(a) \vphi(b) = 0_B \c \vphi(b) = 0_B, \\
      \vphi(ba) &= \vphi(b) \vphi(a) = \vphi(b) \c 0_B = 0_B.
  \end{align*}
  Por lo tanto $ab,ba \in \ker(\vphi)$.
\end{obs}

\begin{exercise}
  Pruebe que si $C$ es subanillo de $B$ y $\vphi \colon A \to B$ es homomorfismo de anillos, entonces $\vphi^{-1}(C)$ es subanillo de $A$.
\end{exercise}

\begin{definition}
  Sea $\phi \colon A \to B$ homomorfismo de anillos. Diremos que
  \begin{enumerate}[(i)]
    \item $\phi$ es \wea{epimorfismo de anillos} si es sobreyectivo.
    \item $\phi$ es \wea{monomorfismo de anillos} si es inyectivo.
    \item $\phi$ es \wea{isomorfismo de anillos} si es biyectivo.
    \item $\phi$ es \wea{endomorfismo de anillos} si $A=B$.
    \item $\phi$ es \wea{automorfismo de anillos} si es isomorfismo y endomorfismo.
  \end{enumerate}
\end{definition}

\begin{exercise}
  Si $\vphi \colon A \to B$ es isomorfismo de anillos, entonces $\vphi(1_A) = 1_B$.
\end{exercise}

\begin{definition}
  Dado un anillo $(R,+,\c)$, definimos el \wea{anillo de series formales} como 
  \[ R[[x]] = R^\N \]
  con la suma componente a componente y la multiplicación de polinomios 
  \[ (a \c b)_n = \sum_{k \leq n} a_k b_{n-k}. \]
\end{definition}

\begin{definition}
  Sea $R$ un anillo. Decimos que $x \in R$ es \wea{nilpotente} si $\exists k \geq 1$ tal que $x^k = 0$.
\end{definition}

\begin{proposition}
  Sea $R$ un anillo. Si $a \in R$ es nilpotente, entonces $1-a$ es unidad. %% demostrar
\end{proposition}

\begin{proposition}
  Sea $R$ anillo conmutativo. Sea 
  \[ p = a_0 + \sum_{i=1}^n a_i x^i. \]
  Entonces $p \in \br{R[x]}^\times \iff a_0 \in R^\times \land a_1,\dots,a_n$ son nilpotentes.
\end{proposition}

\begin{corollary}
  Si $R$ es dominio de integridad, entonces $R^\times \simeq (R[x])^\times$.
\end{corollary}

\begin{obs}
  Si $A,S$ son dos anillos conmutativos, $f \colon A \to S$ es un homomorfismo de anillos y $s \in S$, entonces existe un único homomorfismo de anillo 
  \[ \psi \colon A[x] \to S \]
  tal que $\psi(a) = f(a) \land \psi(x) = s$.
\end{obs}
\begin{proof}
  $A[x] = \gen{A,x}$.
\end{proof}
...
\begin{example}
  Sea $K$ un cuerpo y $p \in K[x]$. Entonces $a$ es raíz de $p \iff x-a \mid p$.
  En particular, $p$ tiene a lo más $\deg(p)$ raíces.
\end{example}

\begin{proposition}
  Sea $K$ un cuerpo y $(G,\c) \leqslant (K^\times,\c)$ subgrupo multiplicativo finito. Entonces existe $n \geq 1$ tal que 
  \[ G \simeq \znz n. \]
\end{proposition}
\begin{proof}
  Como $K$ es cuerpo, es conmutativo. Luego $G$ es abeliano y finito. Por lo tanto existe $g \in G$ tal que $\ord(g) = \exp(G)$. Sea $p \in K[x]$ dado por $p = x^{\exp(G)} - 1$. Este es un polinomio de grado $\exp(G)$, y entonces tiene a lo más $\exp(G)$ raíces.
  Por otro lado, si $h \in G$, entonces 
  \[ p(h) = h^{\exp(G)} - 1 = 0
  \implies \abs G \leq \exp(G) \implies G = \gen g. \]
\end{proof}

\subsection{Ideales y Cocientes}

\begin{definition}
  Sea $(R,+,\c)$ un anillo. Un subconjunto $I \subset R$ se dice
  \begin{enumerate}[(1)]
    \item \wea{Absorbente a la izquierda} si $\forall a \forall x (a \in R \land x \in I \implies ax \in I)$.
    \item \wea{Absorbente a la derecha} si $\forall a \forall x (a \in R \land x \in I \implies xa \in I)$.
  \end{enumerate}
\end{definition}

\begin{definition}
  Sea $(R,+,\c)$ anillo. Un subconjunto $I \subset R$ se dice: 
  \begin{enumerate}[(1)]
    \item \wea{Ideal a la izquierda} si $(I,+) \leqslant (R,+)$ e $I$ es absorbente a la izquierda.
    \item \wea{Ideal a la derecha} si $(I,+) \leqslant (R,+)$ e $I$ es absorbente a la derecha.
    \item \wea{Ideal} (\wea{bilatero}) si cumple (1) y (2).
  \end{enumerate}
\end{definition}

\begin{obs}
  Sea $(R,+,\c)$ un anillo, entonces $\{0\}$ y $R$ son ideales.
\end{obs}

\begin{exercise}
  Sea $(I_\a)_{\a \in \Lambda}$ una colección de ideales (izq,der,bil) de $(R,+,\c)$. Muestre que $\ds \bigcap_{\a \in \Lambda} I_\a$ es ideal (izq,der,bil).
\end{exercise}

\begin{example}
  $R = M_2(\R)$. 
  \[ I = \set{\matr{a & 0 \\ b & 0} \colon a,b \in \R} \]
  $I$ es ideal izquierdo de $M_2(\R)$, pero no es absorbente por la derecha. 
  \[ \matr{1&0\\1&0} \matr{1&1\\1&1} = \matr{1&1\\1&1} \notin I. \]
\end{example}

\begin{proposition}
  Sea $I \subset A$ ideal bilátero, entonces $A/I$ tiene estructura de anillo: Dado $A/I = \{a + I \colon a \in A\}$ 
  \[ (a+I) + (b+I) = (a+b) + I \quad\land\quad (a+I)(b+I) = ab + I. \]
\end{proposition}

\begin{theorem}
  [\wea{Propiedad Fundamental de Ideales}]
  $I \subset A$ es ideal bilátero $\iff \exists \vphi \colon A \to B$ homomorfismo de anillos tal que $I = \ker(\vphi)$.
\end{theorem}
\begin{proof}
  \ \begin{itemize}
    \item \boxed{\impliedby} Ya comprobado.
    \item \boxed{\implies} Consideremos $B = A/I$ con la projección canónica $\pi \colon A \to B$ dado por $\pi(a) = a + I$. Luego $\ker(\pi) = I$.
  \end{itemize}
\end{proof}

\begin{theorem}
  Un anillo $(R,+,\c)$ no trivial es anillo de división si y solo si los únicos ideales izquierdos (y derechos) son los triviales ($\{0\}$ y $R$).
\end{theorem}
\begin{proof}
  \ \begin{itemize}
    \item \boxed{\implies} Supongamos que $(R,+,\c)$ es anillo de división, y sea $I \subset R$ un ideal izquierdo. Supongamos que $I \neq \{0\}$, entonces existe $a \in I \setminus \{0\}$. Como $R$ es de división, tenemos que $a \in R^\times$. Luego $\exists a^{-1}$ tal que $aa^{-1} = 1_R$. Luego por absorción tenemos que $\forall b \in R$, $b = b \c 1_R \in I$. Por lo tanto $R \subset I$, y entonces $I = R$.
    \item \boxed{\impliedby} Supongamos que $R$ es anillo tal que sus únicos ideales (izquierdos) son $\{0\}$ y $R$. Sea $b \in R \setminus \{0\}$. Sea $(b)$ el ideal más pequeño que contiene a $b$ (la intersección de todos los ideales que lo contienen), entonces $(b) = R$.
    
    Notemos que $Rb = \{ab \colon a \in R\}$ es ideal izquierdo que contiene a $b$, y entonces $Rb = R$, y luego $1 \in Rb$, por lo que existe $a \in R$ tal que $ab = 1$, y entonces $b$ es unidad. Como $b$ es arbitrario, tenemos que $R$ es anillo de división.
  \end{itemize}
\end{proof}

\fecha{Clase 17. Lunes 4 de Mayo}

\begin{corollary}
  Si $A$ es un anillo conmutativo, entonces $A$ es cuerpo si y solo si sus únicos ideales son $\{0\}$ y $A$.
\end{corollary}

\begin{obs}
  Un anillo no conmutativo podría no contener ideales biláteros no triviales, y aún así no ser anillo de división.
\end{obs}

\begin{example}
  $M_2(\R)$ no es anillo de división. Sea $I \subset M_2(\R)$ un ideal no trivial, entonces existe $A \in I$ con alguna entrada distinta de 0. Bajo multiplicación por la matriz $\matr{0&1\\1&0}$ podemos permutar las filas entre si, y podemos permutar las columnas entre sí, por lo que podemos asumir que $A_{11} \neq 0$. Luego 
  \[ \matr{1&0\\0&0} A \matr{1&0\\0&0} = \matr{A_{11} & 0 \\ 0 & 0}. \]
  Luego para todo $r \in \R$, tenemos que $\matr{r & 0 \\ 0 & 0} \in I$. Al multiplicar por $\matr{0&1\\1&0}$, obtenemos 
  \[ \matr{r&0\\0&0}, \matr{0&r\\0&0}, \matr{0&0\\r&0}, \matr{0&0\\0&r} \in I, \quad \forall r \in \R. \]
  Luego $I = M_2(\R)$.
\end{example}

\begin{definition}
  Un anillo $A$ se dice \wea{simple} si no contiene ideales biláteros no triviales.
\end{definition}

\begin{obs}
  Si $A$ es anillo conmutativo, entonces $A$ es simple si y solo si $A$ es cuerpo.
\end{obs}

\begin{definition}
  Sea $R$ un anillo. Un ideal $I \subset R$ se dice \wea{maximal} si 
  \[ I \neq R \land \forall J ((I \subset J \subset R) \land J \text{ ideal}) \implies (J = I \lor J = R). \]
\end{definition}

\begin{obs}
  La definición admite cambiar ``ideal'' por ``ideal izquierdo'', ``ideal derecho'' e ``ideal bilátero''.
\end{obs}

\begin{example}
  $R = (\Z,+,\c)$. Los ideales en $R$ son de la forma $I_k = k\Z$, para $k \geq 0$ entero. Tenemos que $I_k$ es ideal maximal si y solo si $k$ es primo.
\end{example}

\begin{definition}
  Sea $A$ un anillo. El \wea{radical de Jacobson} es 
  \[ \mathfrak{J}(A) = \bigcap_{\substack{I \subset A \\ \text{ideal maximal}}} I. \]
\end{definition}

\begin{proposition}
  Sea $R$ anillo no trivial e $I \subset R$ ideal bilátero. $R/I$ es simple $\iff I$ es maximal.
\end{proposition}
\begin{proof}
  $R/I$ es anillo. Sea $J \subset R/I$ ideal bilátero. Consideremos la proyección canónica $\pi \colon R \to R/I$. Entonces $\pi^{-1}(J)$ es ideal bilátero en $R$ y además $I \subset \pi^{-1}(J)$.
  Como $I$ es maximal, entonces $\pi^{-1}(J) \in \{I,R\}$. Luego $J \in \{I,R/I\}$.
\end{proof}

\begin{lemma}
  [Zorn] Sea $(X,\leq)$ un conjunto parcialmente ordenado, con $X \neq \varnothing$.
  Si toda cadena admite cota superior en $X$, entonces $\exists m \in X$ maximal.
\end{lemma}

\begin{theorem}
  Todo anillo admite un cociente simple.
\end{theorem}
\begin{proof}
  Sea $R$ anillo no trivial. Definimos 
  \[ \Ical = \{I \subset R \colon I \text{ es ideal propio}\} \]
  $\Ical \neq \varnothing$ pues $\{0\} \in \Ical$. Dotamos a $\Ical$ del orden parcial $\subset$. Sea $\Gcal \subset \Ical$ una cadena. Consideremos 
  \[ C = \bigcup_{I \in \Gcal} I \]
  por definición, para todo $I \in \Gcal$, tenemos que $I \subset C$. Veamos que $C \in \Ical$:

  Sea $r \in R$ y $a \in C$, entonces existe $I \in \Gcal$ tal que $a \in I$ y entonces $ra \in I$ o $ar \in I$ (dependiendo del tipo de absorbencia que pidamos). Por lo tanto $C$ es absorbente (por el lado que pidamos).

  Si $a,b \in C$, entonces existen $I,J \in \Gcal$ tales que $a \in I$ y $b \in J$. Como $I,J \in \Gcal$, tenemos que $I \subset J$ o $J \subset I$. Sin pérdida de generalidad, $I \subset J$. Luego $a \in J$, y entonces $a+b \in J \subset C$.

  Por lo tanto $C$ es ideal. Si $C = R$, entonces $1 \in C$, luego existe $I \in \Gcal$ tal que $1 \in I$, y entonces $I = R$, contradicción pues $I \in \Ical$. Por lo tanto $C$ es un ideal propio de $R$, y entonces $R \in \Ical$, y es maximal.

  En el caso bilátero, $C$ es un ideal bilátero maximal, y entonces $R/C$ es simple.
\end{proof}

\begin{example}
  Sea $X$ un conjunto arbitrario no vacío y $\R^X = \{f \colon X \to \R\}$. Este es un anillo con suma y producto coordenada a coordenada.
  Sea $Y \subset X$. 
  \begin{enumerate}[(1)]
    \item $I_Y = \{f \in \R^X \colon f(x) = 0, \forall x \in Y\}$ es ideal.
    \item $\abs Y = 1 \implies I_Y$ es maximal.
    \item $\ds \frac{\R^X}{I_{\{y\}}} \simeq \R$.
  \end{enumerate}
\end{example}
\begin{proof}
  \ 
  \begin{enumerate}[(1)]
    \item Trivial.
    \item Sea $Y = \{x_0\}$. Entonces 
    \[ I_Y = \{f \in \R^X \colon f(x_0) = 0\}. \]
    Sea $J$ ideal tal que $I_Y \subsetneq J$. Luego existe $g \in J$ tal que $g(x_0) \neq 0$. Sea $f \in \R^X$. Definimos $h \in \R^X$ dada por 
    \[ h(x) = \begin{cases}
      0, &x = x_0 \\
      -g(x), &x \neq x_0.
    \end{cases} \]
    $h \in I_Y \implies h+g \in J$. Luego $g(x_0)^{-1}(h+g) = \chi_{\{x_0\}} \in J$. Podemos escribir 
    \[ f = \underbrace{f(x_0) \c \chi_{\{x_0\}}}_{\in J} + \underbrace{f \c \chi_{X \setminus \{x\}}}_{\in I_Y \subset J} \in J \]

    \item Definimos $\psi \colon \R \to \R^X/I_{\{x_0\}}$ dada por $\psi(a) = a \chi_{\{x_0\}} + I_{\{x_0\}}$. $\psi$ es isomorfismo. %% demostrar
  \end{enumerate}
\end{proof}
...
\[ \wea{De ahora en adelante, todos los anillos son conmutativos con unidad.} \]

\begin{definition}
  Un anillo $R$ se dice: 
  \begin{enumerate}[(1)]
    \item \wea{Reducido} si el único elemento nilpotente es el 0.
    \item \wea{Local} si tiene un único ideal maximal.
    \item \wea{Dominio de integridad} si no tiene divisores del 0.
  \end{enumerate}
\end{definition}

\begin{example}
  Cualquier cuerpo es reducido, local (el único ideal maximal es $(0)$) y es dominio de integridad.
\end{example}

\begin{example}
  $Z$ es dominio de integridad y reducido. No es un anillo local, ya que $p\Z$ es ideal maximal para todo $p$ primo.
\end{example}

\begin{example}
  $\zmz 6$ es un anillo reducido, pero no es dominio de integridad.
\end{example}

\begin{definition}
  Sea $R$ un anillo. El \wea{nilradical de $R$} es 
  \begin{align*}
      \Nfrak_R &= \sqrt{0} = \rad(0) \\
      &= \{x \in R \colon x \text{ es nilpotente}\}
  \end{align*}
\end{definition}

\begin{obs}
  $\Nfrak_R$ es un ideal de $R$.
\end{obs}

\begin{proposition}
  $R/\Nfrak_R$ es un anillo reducido.
\end{proposition}
\begin{proof}
  Sea $a + \Nfrak_R$ nilpotente en $R/\Nfrak_R$. Entonces existe $k \geq 1$ tal que $a^k + \Nfrak_R = \Nfrak_R$. Luego $a^k \in \Nfrak_R = \rad(0)$, y entonces $a \in \rad(0)$. Por lo tanto $a + \Nfrak_R = \Nfrak_R = 0_{R/\Nfrak_R}$.
\end{proof}

\begin{definition}
  Un ideal $I \subset R$ se dice \wea{primo} si 
  \[ I \neq R \land \forall a \forall b \br{ab \in I \implies (a \in I \lor b \in I)} \]
\end{definition}

\begin{theorem}
  $$\rad(0) = \bigcap_{\substack{I \subset R \\ \text{ideal primo}}} I.$$
\end{theorem}
...
\begin{example}
  Sea $R$ anillo, y sea $I \subset R$ ideal. Entonces $R/I$ es reducido si y solo si existe $J \subset R$ ideal tal que $I = \rad(J)$.
\end{example}
...
\begin{proposition}
  $\znz{p^n}$ es anillo local para todo $p$ primo y $n \geq 1$.
\end{proposition}

\begin{proposition}
  Sea $R$ un anillo no trivial. Son equivalentes: 
  \begin{enumerate}[(1)]
    \item $R$ es local.
    \item $R \setminus R^\times$ es el ideal maximal de $R$.
    \item $a \in R \implies (a \in R^\times \lor 1-a \in R^\times)$.
  \end{enumerate}
\end{proposition}
\begin{proof}
  \ \begin{itemize}
    \item \boxed{(1) \implies (2)} Supongamos que $R$ es local. Sea $I$ el ideal maximal. Sea $a \in R \setminus R^\times$. Notemos que 
    \[ (a) = \bigcap_{\substack{I \text{ ideal} \\ a \in I}} I = Ra = \{xa \colon x \in R\}. \]
    Como $a \notin R^\times$, entonces $(a) \neq R$. Por lo tanto $(a) \subset I$, y entonces $a \in I$. Por lo tanto $R \setminus R^\times \subset I$.
    Como $I \cap R^\times  = \varnothing$, entonces $I = R \setminus R^\times$.
    \item \boxed{(1) \impliedby (2)} Ejercicio %% mejorar por pasos reversibles.
    \item \boxed{(2) \iff (3)} Ejercicio. %% demostrar todos estos.
  \end{itemize}
\end{proof}

\begin{obs}
  Si $R$ es local, entonces el ideal maximal $R \setminus R^\times$ es el de Jacobson.
\end{obs}

¿Por qué se llaman ``anillos locales''?
Hay 2 razones: 
\begin{enumerate}[(1)]
\item \wea{Anillos de gérmenes}. Consideremos 
\[ \Ccal(\R) = \{f \in \R^\R \text{ continuas}\} \]
y consideremos la relación de equivalencia $f \sim g \iff \exists V$ vecindad del 0 tal que $f|_V = g|_V$.
Escribimos $\Gcal = \Ccal(\R)/{\sim}$ y observamos que es un anillo con la suma y multiplicación de funciones.
\begin{obs}
  $f \in \Gcal$ es invertible $\iff f(0) \neq 0$.
\end{obs}
Notemos que $\{f \in \Gcal \colon f(0) = 0\}$ es un ideal. Entonces $\Gcal$ es anillo local.

\item \wea{Localizaciones}.
\begin{definition}\footnote{El profesor solo definió el conjunto $S^{-1}R$ sin hablar de la relación de equivalencia. Toda la construcción formal de fracciones aparece más adelante al construir $\Frac(R)$, pero igual añadí lo necesario acá por completitud del apunte.}
  Dado un anillo $R$ y $S \subset R \setminus \{0\}$ un conjunto cerrado por multiplicación (i.e., $(S,\c) \subset (R,\c)$ semigrupo conmutativo), consideramos la relación de equivalencia en $R \times S$ dada por
  \[ (a,s) \sim (b,t) \iff \exists u \in S, u(at - bs) = 0. \]
  Definimos $S^{-1}R = (R \times S)/{\sim}$.
  \[ \frac{a}{b} = [(a,b)] \]
  y definimos las operaciones 
  \[ \frac{a}{s} + \frac{b}{t} = \frac{at + bs}{st}, \quad \frac{a}{s} \c \frac{b}{t} = \frac{ab}{st}. \]
  Luego
  \[ S^{-1}R = \set{\frac{a}{b} \colon a \in R, b \in S}. \]
\end{definition}

\begin{example}
  $R = \Z$, $S = \{n \in \Z \colon 2 \nmid n\} = 1 + 2\Z$. Tenemos que 
  \[ S^{-1}\Z = \set{\frac{a}{b} \in \Q \colon a,b \in \Z , 2 \nmid b.} \]
\end{example}

\begin{obs}
  Si $R \setminus S$ es un ideal primo, entonces $S^{-1} R$ es un anillo local.
\end{obs}
\end{enumerate}


\subsection{Dominios de Integridad}

\begin{definition}
  $R$ es \wea{dominio de integridad} (abreviado como \wea{DI}) si $\forall a,b \in R (ab = 0 \implies (a=0 \lor b=0))$.
\end{definition}

\begin{obs}
  Si $R$ es DI $\implies R[x]$ es DI.
\end{obs}

\begin{proposition}
  $R$ es DI $\iff (0)$ es primo.
\end{proposition}

\begin{obs}
  $R$ es DI $\implies R$ es cancelativo.
\end{obs}
\begin{proof}
  Sean $a,b,c \in R$ tales que $ab = ac$, entonces $a(b-c) = 0$, y entonces $a=0$ o $b=c$.
\end{proof}

\begin{theorem}
  Sea $R$ un DI finito. Entonces $R$ es cuerpo.
\end{theorem}
\begin{proof}
  Sea $a R \setminus \{0\}$. Definimos $\lam_a \colon R \to R$ dado por $\lam_a(x) = ax$.
  Como $R$ es DI, $\lam_a$ es inyectiva. Como $\abs R < \oo$, entonces $\lam_a$ es sobreyectiva. Luego existe $x \in R$ tal que $ax = 1$, y entonces $a \in R^\times$.
\end{proof}

\begin{definition}
  Sea $R$ anillo. La \wea{característica de $R$} es:
  \begin{itemize}
    \item Si $\gen{1} \leqslant (R,+)$ es finito, 
    \[ \Char(R) = \min\set{n \in \N \colon \sum_n 1 = 0}. \]
    \item Si $\gen{1} \leqslant (R,+)$ es infinito, $\Char(R) = 0$.
  \end{itemize}
\end{definition}

\begin{proposition}
  Si $R$ es DI, entonces $\Char(R)$ es primo o 0.
\end{proposition}
\begin{proof}
  Sea $n > 0$ tal que $\Char(R) = n$. Si $n$ no es primo, entonces $n = pq$, con $1 < p,q < n$ enteros. Luego 
  \[ 0 = \sum_{pq} 1 = \br{\sum_p 1} \br{\sum_q 1} \]
  y entonces $\sum_p 1 = 0$ o $\sum_q 1 = 0$, por lo que $\Char(R) < n$. \(\to\gets\)
\end{proof}

Ahora, dado $R$ un DI, queremos encontrar algún cuerpo $K$ tal que $R \embed K$.

\begin{definition}
  Sea $R$ un DI. Consideramos la relación de equivalencia $\sim$ en $R \times R \setminus \{0\}$ dada por 
  \[ (a,b) \sim (c,d) \iff ad = bc. \]
  Definimos el \wea{cuerpo de fracciones de $R$} como 
  \[ \Frac(R) = \frac{R \times (R \setminus \{0\})}{\sim}. \]
\end{definition}

\begin{obs}
  $\sim$ es relación de equivalencia. Denotamos 
  \[ [(a,b)]_\sim = \frac{a}{b}. \]
\end{obs}

\begin{definition}
  En $\Frac(R)$, definimos 
  \[ \frac{a}{b} + \frac{c}{d} = \frac{ad+bc}{bd}, \quad \frac{a}{b} \c \frac{c}{d} = \frac{ac}{bd}. \]
\end{definition}

\begin{obs}
  Estas son operaciones bien definidas.
\end{obs}
...
\end{theorem}
\begin{proof}
  Las operaciones son cerradas, asociativas y conmutativas. El neutro es $0/1$ y la unidad es $1/1$.
  El inverso multiplicativo de $a/b$ es $b/a$ (si $a/b \neq 0/1$). El inverso aditivo de $a/b$ es $(-a)/b$. Para la distributividad: 
  \begin{align*}
      \frac{a}{b} \br{\frac{c}{d} + \frac{e}{f}}
      &= \frac{a}{b} \br{\frac{cf+de}{df}} \\
      &= \frac{acf + ade}{bdf} \\
      &= \frac{acf + ade}{bdf} \c \frac{1}{1} \\
      &= \frac{acf + ade}{bdf} \c \frac{b}{b} \\
      &= \frac{(acf + ade)b}{(bdf)b} \\
      &= \frac{(acf)b + (ade)b}{(bdf)b} \\
      &= \frac{(ac)(bf) + (ae)(bd)}{(bd)(bf)} \\
      &= \frac{ac}{bd} + \frac{ae}{bf} \\
      &= \frac{a}{b} \c \frac{c}{d} + \frac{a}{b} \c \frac{e}{f}
  \end{align*}
\end{proof}

\begin{theorem}
  [\wea{Propiedad Fundamental del Cuerpo de Fracciones}] Si $K$ es cuerpo y $R \embed K$ es un DI, entonces $\Frac(R) \embed K$.
\end{theorem}

\begin{obs}
  $R \embed \Frac(R)$ por medio de $x \mapsto x/1$. i.e.: $\Frac(R)$ es el cuerpo más pequeño que contiene a $R$.
\end{obs}
\begin{proof}
  Sea $\vphi \colon R \to K$ monomorfismo. Definimos $\tilde\vphi \colon \Frac(R) \to K$ dada por $\tilde\vphi(a/b) = \vphi(a) \vphi(b)^{-1}$. Esta función está bien definida: Si $a/b = c/d$, entonces $ad = bc$, por lo que 
  \begin{align*}
      \frac{a}{b} = \frac{c}{d}
      &\iff ad = bc \\
      &\implies \vphi(a)\vphi(d) = \vphi(b) \vphi(c) \\
      &\implies \vphi(a)\vphi(b)^{-1} = \vphi(c)\vphi(d)^{-1} \\
      &\implies \tilde\vphi\br{\frac{a}{b}} = \tilde\vphi\br{\frac{c}{d}}.
  \end{align*}
  $\tilde\vphi$ es inyectiva y homomorfismo. %% Me faltó escribir la demostración. Se hizo en ese orden.
\end{proof}

\begin{corollary}
  Si $K$ es cuerpo, y $\Char(K) = 0$, entonces $\Q \embed K$.
\end{corollary}

\fecha{Clase 19. Lunes 11 de Mayo}

\begin{theorem}
  [\wea{Teorema Chino del Resto (Versión Clásica)}]
  Sean $n_1,\dots, n_k,a_1,a_k$ enteros con $n_i > 1$, $0 \leq a_i < n_i$.
  Sea $N = n_1 \cdots n_k$. Si $\gcd(n_i,n_j) = 1$ cuando $i \neq j$, entonces existe un único $x$ tal que $0 \leq x < N$ y $x \equiv a_i \bmod n_i$ para todo $i$.
\end{theorem}

\begin{definition}
  Sea $R$ un anillo e $I,J \subset R$ ideales. Decimos que $I$ y $J$ son \wea{coprimos} si $I + J = (1)$.
\end{definition}

\begin{theorem}
  [\wea{Teorema Chino del Resto (Versión Moderna)}]
  Sea $R$ un DI e $I_1, \dots, I_n$ ideales coprimos. Entonces 
  \[ \frac{A}{(I_1 \cdots I_n)} \simeq \frac{A}{I_1} \times \dots \times \frac{A}{I_n} \]
  donde $I_1 \cdots I_n$ es el ideal generado por elementos de la forma $a_1a_2 \cdots a_n$ con $a_i \in I_i$.
\end{theorem}

\begin{example}
  Si tomamos $R = \Z$, si tenemos $n_1,\dots,n_k > 1$ son coprimos de a pares, entonces los ideales $(n_1), \dots, (n_k)$ son coprimos de a pares. Luego 
  \[ \znz N = \znz{(n_1 \cdots n_k)} \simeq \znz{n_1} \times \dots \times \znz{n_k}. \]
\end{example}

\begin{proposition}
  Sea $R$ un DI e $I$ un ideal. $I$ es ideal primo $\iff \forall A,B \subset R$ ideales, $$(AB) \subset I \implies A \subset I \lor B \subset I.$$
\end{proposition}
...
\begin{definition}
  Sea $a \in R \setminus R^\times$. Decimos que $a$ es \wea{primo} si $\forall b,c \in R$, $$a \mid bc \implies a \mid b \lor a \mid c.$$
\end{definition}

\begin{definition}
  Un ideal $I \subset R$ se dice \wea{principal} si $\exists a \in R$ tal que $I = (a)$.
\end{definition}

\begin{proposition}
  $a$ es primo $\iff (a)$ es primo.
\end{proposition} %% demostrar como ejercicio personal

\begin{proposition}
  Sea $R$ anillo conmutativo con unidad e $I \ubset R$ un ideal. Si $I$ es primo, entonces $R/I$ es DI.
\end{proposition}

\newpage %% mdframed bad break. arreglar
\begin{exercise}
  Si $I$ es ideal maximal, entonces es ideal primo.
\end{exercise}
\begin{proof}
  Por contradicción, supongamos que $I$ es maximal, pero no es primo. Entonces existen $a,b \in R$ tales que $ab \in I$, pero $a,b \notin I$. Como $I$ es maximal, tenemos que 
  \[ (a,I) = R \]
...
\begin{exercise}
  Describir ideales primos en términos de ideales coprimos.
\end{exercise}

\begin{definition}
  Sea $a \in R \setminus R^\times$. Decimos que \wea{irreducible} si 
  \[ a = bc \implies b \in R^\times \lor c \in R^\times. \]
\end{definition}

\begin{definition}
  Sea $R$ un DI. Un ideal $I \neq R$ se dice \wea{irreducible} si $\forall J,J'$ ideales, 
  \[ I = J \cap J' \implies I=J \lor I=J'. \]
\end{definition}

\begin{obs}
  Para DI no es cierto que $a$ irreducible $\iff (a)$ irreducible. Por ejemplo, con $R = \Z$ consideramos $a = 4$. No es irreducible, ya que $4 = 2 \c 2$, y $2 \notin \Z^\times$. Pero $(4) = 4\Z$ es irreducible.
\end{obs}

\begin{proposition}
  Los ideales primos son ideales irreducibles.
\end{proposition}

\begin{proposition}
  Sea $R$ un DI y $a \in R$. Si $a$ es primo, entonces es irreducible.
\end{proposition}

\begin{definition}
  Un DI $R$ se dice \wea{dominio de ideales principales} (abreviado como \wea{PID} o \wea{DIP}) si todo ideal es principal.
\end{definition}

\begin{example}
  Sea $R$ un DIP y sea $a \in R$. $a$ es irreducible $\iff (a)$ es maximal.
\end{example} %% demostrar

\begin{corollary}
  Si $R$ es DIP, entonces son equivalentes: 
  \begin{enumerate}[(1)]
...
\begin{obs}
  En un DI algunas implicancias fallan. Por ejemplo:
  \begin{itemize}
    \item $x \in \Z[x]$ es irreducible y primo, pero $(x)$ no es maximal, ya que $\Z[x]/(x) \simeq \Z$ no es cuerpo.
    \item En $\Z[\sqrt{-5}] = \{a + bi \sqrt{5} \in \C \colon a,b \in \Z\}$, 3 es irreducible pero no es primo, pues 
    \[ 6 = 2 \ 3 = (1+i\sqrt5)(1-i\sqrt5). \]
    Notemos que $\forall z,z' \in \C$, $\abs{zz'} = \abs z \c \abs{z'}$. Si $z = a+bi$, entonces $\abs z = \sqrt{a^2+b^2}$. Si $u \in \Z[i\sqrt5]$ es unidad, entonces $\abs u = 1$, y entonces $u \in \{1,-1\}$. Luego 3 no puede dividir a $1+i\sqrt5$ ni a $1 - i \sqrt5$. Por lo tanto 3 no es primo. %% demostrar que 3 es irreducible. Hint: use la norma
  \end{itemize}
\end{obs}

\fecha{Clase 20. Miércoles 13 de Mayo}

\begin{definition}
  Un DI $R$ se dice \wea{dominio de factorización única} (abreviado \wea{DFU} o \wea{UFD}) si todo $a \in R \setminus (R^\times \cup \{0\})$ se escribe de manera única (salvo unidades) como producto de irreducibles. i.e.: $\exists b_1,\dots,b_k \in R$ irreducibles tal que 
  \[ a = b_1 \cdots b_k \]
...
\begin{exercise}
  Mostrar que $\Z[x]$ es DFU.
\end{exercise}

\begin{proposition}
  Sea $R$ un DFU y $a \in R$ irreducible. Entonces $a$ es primo.
\end{proposition}
...
\begin{definition}
  Un anillo $R$ se dice \wea{Noetheriano} si toda secuencia ascendente de ideales $(I_n)_{n \geq 1}$ se estabiliza. i.e.: Si $I_n \subset I_{n+1}$ para todo $n \geq 1$, entonces existe $n_0$ tal que $I_n = I_{n_0}$ para todo $n \geq n_0$.
\end{definition}
...
\begin{definition}
  Sea $R$ un DI y $S \subset R$ finito.
  \begin{itemize}
    \item Decimos que $a \in R$ es \wea{máximo común divisor de $S$} (abreviado como \wea{gcd}) si
    \[ \forall b \in R (\forall s \in S (b \mid s) \implies b \mid a). \]
    \item Decimos que $a \in R$ es \wea{máximo común múltiplo de $S$} (abreviado como \wea{lcm}) si 
    \[ \forall b \in R (\forall s \in S(s \mid b) \implies a \mid b). \]
  \end{itemize}
\end{definition}

\newpage
\fecha{Clase 21. Lunes 1 de Junio}
\subsubsection{Dominios Euclideanos}

\begin{definition}
  Un DI $R$ se dice \wea{dominio Euclideano} (abreviado como \wea{DE}) si $\exists \nu \colon R \setminus \{0\} \to \N$ tal que 
  \begin{enumerate}[(i)]
    \item $\forall a,b \neq 0, \quad \nu(a) \leq \nu(ab)$.
    \item $\forall a,b \in R \setminus \{0\}$, $\exists q,r \in R$ tal que $a = qb+r$ con $r = 0$ o $\nu(r) < \nu(b)$.
  \end{enumerate}
\end{definition}

\begin{example}
  $\Z$ es un DE con el valor absoluto.
\end{example}

\begin{proposition}
  Si $k$ es un cuerpo, entonces $k[x]$ es DE.
\end{proposition}
\begin{proof}
  Tomamos $\nu(p(x)) = \deg{p(x)}$. La hipótesis de cuerpo sirve para el algoritmo de la división.
\end{proof}

\begin{obs}
  Con esto tenemos que todo $p(x) \in k[x]$ tiene a lo más $\deg{p}$ raíces. Esto no funciona siempre en anillos que no sean cuerpos. Por ejemplo, $x^2 - 1 \in \zmz8[x]$ tiene 3 raíces: 1, 7 y 3.
\end{obs}

\begin{lemma}
  Sea $R$ un DE con valuación $\nu$. Entonces 
  \[ x \in R^\times \iff \nu(x) = \min_{r \in R \setminus \{0\}} \nu(r). \]
\end{lemma}
\begin{proof}
  \ \begin{itemize}
    \item \boxed{\impliedby} Si $x$ tiene norma mínima, entonces por el algoritmo de división con 1, tenemos que existen $q,r \in R$ tales que 
    \[ 1 = qx+r \]
    con $r = 0$ o $\nu(r) < \nu(x)$, pero como la norma de $x$ es mínima, tenemos que $r = 0$, y entonces $qx = 1$. Por lo tanto $x \in R^\times$.
    \item \boxed{\implies} Si $x \in R^\times$, entonces tiene inverso $x^{-1}$. No me acuerdo como continua la demostración fkgjfkgjk %% completar.
  \end{itemize}
\end{proof}
...
\end{proposition}
\begin{proof}
  Sea $A$ un DE y sea $I \subseteq A$ un ideal. Como la imagen de $\nu$ es discreta, existe $a_0 \in I$ tal que $\nu(a_0) \leq \nu(a)$ para todo $a \in I \setminus \{0\}$. Sea $b \in I \setminus \{0\}$, entonces existen $q,r \in R$ tales que 
  \[ b = qa_0 + r \]
  Con $r = 0$ o $\nu(r) < \nu(a_0)$. Notemos que $r = 0$, ya que en el caso contrario tendríamos $r = b - qa_0 \in I$ con valuación menor a $a_0$. Por lo tanto $b = qa_0$ y entonces $b \in (a_0)$. Luego $I \subseteq (a_0)$, y entonces $I = (a_0)$.
\end{proof}
...
\end{proposition}
\begin{proof}
  El ideal $(2,x) \subseteq \Z[x]$ no es principal. Entonces no es DIP, y entonces no es DE.
\end{proof}

\begin{exercise}
  Mostrar que $\R[x,y]$ no es DE.
\end{exercise}

\begin{example}
  $\Z[i]$ es un DE, con $\nu = N^2$ donde $N \colon \C \to \R$ es la norma usual: $\nu(a+bi) = a^2 + b^2$.
\end{example}

\begin{obs}
  $N^2(zw) = N^2(z) N^2(w)$. Como $N^2$ toma valores positivos en $\Z[i] \setminus \{0\}$, $N^2$ cumple la propiedad $N^2(z) \leq N^2(zw)$. 
\end{obs}

\begin{example}
  $\Z[\sqrt{-5}]$ no es DFU. Además no es DE.
\end{example}

\fecha{Clase 22. Miércoles 3 de Junio}

\begin{definition}
  Sea $A$ un DI y $a,b \in A$. Decimos que $d \in A$ es un \wea{máximo común divisor (mcd) de $a$ y $b$} si
  \[ d \mid a \ \land \ d \mid b \ \land \ \forall t \ \br{(t \mid a \land t \mid b) \implies t \mid d}. \]
\end{definition}

\begin{theorem}
  [Euclides]
  Sea $A$ un DE y sean $a,b \in A \setminus \{0\}$. Definimos $q_i$ y $r_i$ inductivamente por
  \begin{align*}
      a &= q_0 b + r_0, \quad \nu(r_0) < \nu(b) \\
      b &= q_1 r_0 + r_1, \quad \nu(r_1) < \nu(r_0) \\
      r_0 &= q_2 r_1 + r_2, \quad \nu(r_2) < \nu(r_1) \\
      &\vdots \\
...
\begin{exercise}
  Buscar mcd de $q(x) = x^3 + 3x^2 + x$ y $b(x) = x^3 - x$ en $\Q[x]$.
\end{exercise}

\begin{theorem}
  $\Z\cor{\frac{1+\sqrt{-19}}{2}}$ es DIP, pero no es DE.
\end{theorem}

\newpage
\fecha{Clase 23. Lunes 8 de Junio}
\subsection{Anillos de Polinomios}

Motivación: Criterio de la raíz racional.

Sea $h = \sum_{i=1}^n a_i x^i \in \Z[x]$, con $a_n,a_0 \neq 0$.
Si $p/q \in \Q$ es tal que $h(p/q) = 0$, con $\gcd(p,q) = 1$, entonces $p \mid a_0 \ \land \ q \mid a_n$.

Esto nos da un algoritmo en el que dado $h \in \Z[x]$ se encuentran todas sus raíces racionales.

¿Existe un algoritmo que en entrada $h \in \Z[x_1,\dots,x_5]$ decida si admite una raíz racional? Este es un problema abierto. Este es el décimo problema de Hilbert.

Vamos a probar algo más general que el criterio de la raíz racional:

\begin{definition}
  Sea $A$ un anillo conmutativo e $I \subset A$ un ideal. Denotamos por $IA[x]$ el ideal inducido por $I$ en $A[x]$:
  \[ IA[x] = \set{\sum_{i=0}^n a_i x^i : a_i \in I}. \]
\end{definition}

\begin{obs}
  Si consideramos el homomorfismo proyección $\pi \colon A \to A/I$, este se extiende a $\bar\pi \olon A[x] \to (A/I)[x]$ mediante
  \[ \bar\pi\br{\sum_{i=0}^n a_i x^i} = \sum_{i=0}^n \pi(a_i)x^i. \]
  Como $\pi$ es sobreyectiva, tenemos que $\bar\pi$ también es sobreyectiva. Notemos que
  \begin{align*}
      \sum_{i=0}^n a_i x^i \in \ker\bar\pi
      &\iff \bar\pi\br{\sum_{i=0}^n a_i x^i} = 0 \\
      &\iff \sum_{i=0}^n \pi(a_i) x^i = 0 \\
      &\iff \pi(a_i) = 0, \quad \forall i \\
      &\iff a_i \in I, \quad \forall i \\
      &\iff \sum_{i=0}^n a_i x^i \in IA[x].
  \end{align*}
  Por lo tanto $\ker\bar\pi = IA[x]$. Luego por el primer teorema de isomorfismo
  \[ \frac{A[x]}{IA[x]} \simeq \br{\frac{A}{I}}[x]. \]
\end{obs}

\begin{example}
  Sea $A$ anillo conmutativo y $a \in A$. Si $a$ es primo en $A$, entonces $a$ es primo en $A[x]$.
\end{example}
\begin{proof}
  \begin{align*}
    \text{$a$ es primo en $A$}
    \iff &\text{$(a)$ es ideal primo de $A$} \\
    \iff &\text{$A/(a)$ es DI} \\
    \implies &\frac{A}{(a)}[x] \text{ es DI} \\
    \implies & \frac{A[x]}{(a)A[x]} \text{ es DI} \\
    \implies & \text{$(a)A[x]$ es ideal primo} \\
    \implies &\text{$a$ es primo en $A[x]$}
  \end{align*}
\end{proof}

\begin{definition}
  Sea $A$ un DFU. Diremos que $f \in A[x]$ es \wea{primitivo} si el MCD de sus coeficientes es una unidad (i.e.: 1 es un MCD de ellos).
\end{definition}

\begin{lemma}
  [Gauß]
  Sea $A$ un DFU y sea $F = \Frac(A)$. Supongamos que $\deg{f} \geq 1$. Entonces $f$ es irreducible en $A[x]$ si y solo si $f$ es primitivo en $A$ y $f$ es irreducible en $F[x]$.
\end{lemma}

\begin{lemma}
  Sea $A$ un DFU y $F = \Frac(A)$. Sea $f \in A[x]$ con $\deg{f} \geq 1$. Si $f$ es reducible como elemento de $F[x]$, entonces $f$ es reducible como elemento de $A[x]$.
\end{lemma}

\begin{theorem}
  Si $A$ es DFU, entonces $A[x]$ es DFU.
\end{theorem}

\fecha{Clase 24. Miércoles 10 de Junio}

\subsection{Criterio de Eisenstein}

Dado $p$ primo, denotamos $\mathrm{GF}(p) = \Z_p$ el cuerpo de $p$ elementos. Existe una proyección
\[ \psi_p \colon \Z[x] \to \mathrm{GF}(p)[x] \]
dada por
\[ \psi_p\br{\sum_{i=1}^n a_i x^i} = \sum_{i=1}^n [a_i]_p x^i. \]

\begin{lemma}
  Sea $p$ primo y sea $\ds f = \sum_{i=1}^n a_i x^i \in \Z[x]$ tal que $a_n \not\equiv 0 \mod{p}$. Si $f$ es irreducible en $\mathrm{GF}(p)[x]$, entonces $f$ es irreducible en $\Q[x]$.
\end{lemma}

\begin{theorem}
  [Criterio de Eisenstein]
  Sea $\ds f = \sum_{i=1}^n a_i x^i \in \Z[x]$ y $p$ primo tal que
  \begin{enumerate}[(i)]
    \item $a_n \not\equiv 0 \mod{p}$,
    \item $p \mid a_{n-1}, \dots, p \mid a_0$,
    \item $a_0 \not\equiv 0 \mod{p^2}$,
  \end{enumerate}
...
\begin{theorem}
  Sea $R$ un DIP, y sea $q = \sum_{i=0}^n a_i x^i \in R[x]$. Sea $\mathfrak{p}$ ideal primo tal que
  \begin{enumerate}[(i)]
    \item $a_n \notin \mathfrak p$,
    \item $a_i \in \mathfrak{p}, \forall i \neq n$,
    \item $a_0 \notin \mathfrak{p}^2$.
  \end{enumerate}
  entonces $q$ no es producto de dos polinomios no constantes en $R[x]$.
  Si además $R$ es DFU y $q$ es primitivo, entonces $q$ es irreducible en $R[x]$.
\end{theorem}

\subsection{Anillos Noetherianos y Artinianos}

\begin{definition}
  $R$ se dice \wea{Noetheriano} si toda cadena ascendente de ideales se estabiliza, y se dice \wea{Artiniano} si toda cadena descendente se estabiliza.
\end{definition}

\begin{obs}
  Si $R$ es Artiniano, DI y conmutativo, entonces es un cuerpo.
\end{obs}

\begin{theorem}
  [Wedderburn--Artin]
  Todo anillo simple Artiniano es isomorfo a un anillo de matrices con coeficientes en un anillo de división.
\end{theorem}
...
\fecha{Clase 25. Lunes 15 de Junio}

\begin{definition}
  Dados dos cuerpos $K$ y $F$, decimos que $K$ es \wea{extensión de $F$} si $F$ es subcuerpo de $K$.\footnote{Todavía no hablaremos de extensiones modulo isomorfismo.}
  Se denota $K/F$.
\end{definition}

\begin{obs}
  Para toda extensión $K/F$ se tiene que $K$ es un $F$-espacio vectorial.
\end{obs}

\begin{definition}
  El \wea{grado} de una extensión $K/F$ es la dimensión de $K$ como $F$-espacio vectorial:
  \[ [K : F] = \dim_F(K). \]
\end{definition}
...
\begin{definition}
  Sea $K/F$ extensión de cuerpos. Diremos que $\a \in K$ es \wea{algebraico sobre $F$} si existe $p \in F[x]$ tal que $p(\a) = 0$.
  Si $\a$ no es algebraico, se dice \wea{trascendente sobre $F$}.
\end{definition}

\begin{obs}
  Si $\a$ es algebraico, entonces existe un polinomio mónico que lo anula.
\end{obs}

\begin{obs}
  Si $\a \in K$ es algebraico sobre $F$, existe un único polinomio mónico $p_\a = \min(\a,F) \in F[x]$ de grado mínimo tal que $p_\a(\a) = 0$. Este polinomio de denomina \wea{polinomio minimal de $\a$ sobre $F$}.
\end{obs}

\begin{exercise}
  Pruebe que $\a \in K$ es algebraico sobre $F$ si y solo si el $F$-espacio vectorial generado por $\{1,\a,\a^2, \dots\}$ no es linealmente independiente sobre $F$.
\end{exercise}

\begin{obs}
  $\a \in K$ es algebraico sobre $F$ si y solo si $\exists p_\a \in F[x]$ tal que $p_\a(\a) = 0$.
  Sea $\ds p_\a = \sum_{i=0}^n b_i x^i$ con $b_i \in F$, $b_n = 1$. Evaluando en $\a$ obtenemos
  \[ \sum_{i=0}^n b_i \a^i = 0 \iff \a^{n+k} = - \sum_{i=0}^{n-1} b_i \a^{i+k}. \]
  Luego $\dim_F \gen{\{\a^i : i \in \N\}} = \deg(p_\a)$.
\end{obs}

\begin{obs}
  En términos de anillos, si tenemos una extensión de cuerpos $K/F$, consideramos el homomorfismo de evaluación en $\a \in K$
  \begin{align*}
      \varphi_\a \colon F[x] &\to K \\
      p &\mapsto p(\a).
  \end{align*}
  Tenemos que $\a$ es algebraico sobre $F$ si y solo si $\ker\varphi_\a \neq \{0\}$. Como $F$ es cuerpo, entonces $F[x]$ es DE, y entonces es DIP. Por lo tanto existe $p \in F[x]$ tal que $\ker\vphi_\a = (p)$. Módulo unidades, tenemos que $p = \min(\a,F)$.
\end{obs}

\newpage %% mdframed, arreglar eventualmente
\begin{obs}
  Sea $\a$ algebraico en $K/F$. Entonces $p_\a = \min(\a,F)$ es irreducible.
\end{obs}
\begin{proof}
  Si $p_\a = qq'$ con $q,q' \in F[x]$, entonces $0 = p_\a(\a) = q(\a) q'(\a)$. Como $F$ es cuerpo, tenemos que $q(\a) = 0 \ \lor \ q'(\a) = 0$. Sin pérdida de generalidad, $q(\a) = 0$. Como $p_\a$ tiene grado minimal, tenemos que $\deg p_\a = \deg q$, y entonces $\deg q' = 0$. Por lo tanto $q' \in (F[x])^\times$. Luego $p_\a$ es irreducible.
\end{proof}

\begin{obs}
  Sea $F$ cuerpo y $p \in F[x]$ con $2 \leq \deg p \leq 3$. $p$ es irreducible si y solo si $p$ no tiene raíces en $F$.
\end{obs}

\begin{obs}
  Esto es falso si $\deg p \geq 4$. Por ejemplo, $p = x^4 + 1 \in \R[x]$. Este polinomio es reducible en $\R[x]$:
  \[ x^4 + 1 = (x^2 + \sqrt{2} x + 1)(x^2 - \sqrt{2} x + 1). \]
\end{obs}

\begin{definition}
  Sea $K$ cuerpo y $A \subset K$ subanillo y $S \subset K$ conjunto. El \wea{subanillo de $K$ generado por $A$ y $S$} es
  \[ A[S] = \bigcap_{\substack{A \cup S \subset R \subset K \\ \text{anillo}}} R. \]
  El \wea{subcuerpo de $K$ generado por $A$ y $S$} es
  \[ A(S) = \bigcap_{\substack{A \cup S \subset L \subset K \\ \text{cuerpo}}} L. \]
\end{definition}

\begin{proposition}
  $$A[S] = \set{\sum_{i=1}^n a_i s_{1,i} \dots s_{k_i,i} : k_i, n \in \N, a_i \in A, s_{j,i} \in S}.$$
\end{proposition}

\begin{definition}
  Si $S = \{b\}$, escribimos
  \[ A[\{b\}] = A[b] = \set{\sum_{i=1}^n a_i b^i : n \in \N, a_i \in A}. \]
\end{definition}
...
\begin{example}
  $\Q[i] = \{a + bi : a,b \in \Q\} = \Q(i)$.
\end{example}

\begin{example}
  $\Q[i\sqrt{5}] = \{a + ib \sqrt{5} : a,b \in \Q\} = \Q(\sqrt{5})$.
\end{example}

\begin{proposition}
  Sea $\a \in K$ algebraico sobre $F$, entonces $F[\a]$ es cuerpo y $[F[\a] : F] = \deg(\min(\a,F))$.
\end{proposition}
\begin{proof}
  Considere $\psi_\a \colon F[x] \to K$ el homomorfismo de evaluación en $\a$. Luego
  \[ F[\a] = \psi_\a(F[x]) \simeq \frac{F[x]}{\ker\psi_\a} = \frac{F[x]}{(p_\a)} \]
  donde $p_\a = \min(\a,F)$.
  Si $\exists I$ ideal tal que $(p_\a) \subsetneq I \subsetneq F[x]$, entonces $I = (q)$ pues $F[x]$ es DIP. Luego $q \mid p_\a = p_\a = q \cdot t$ con $\deg t \geq 1$. Pero $p_\a$ es irreducible. \(\to\gets\)

  Por lo tanto $(p_\a)$ es maximal, y entonces $F[x]/(p_\a) \simeq F[\a]$ es cuerpo.

  Ejercicio: grado de la extensión.
\end{proof}

\begin{obs}
  En general, si $F$ es cuerpo y $p \in F[x]$ es irreducible, entonces $F[x]/(p)$ es cuerpo.
\end{obs}
...
\begin{example}
  Sea $F = \F_2 = \mathrm{GF}(2)$ y $p = x^2 + x + 1$. Este polinomio es irreducible. Luego $K = F[x]/(x^+ x + 1)$ es cuerpo. Tenemos que $[K:F] = 2$, y entonces $\abs K = 4$. Notemos que $(\F_2,+)$ es un grupo abeliano de característica 2, por lo que $(\F_2,+) \simeq \Z_2 \times \Z_2$. Además, $(\F_2 \setminus \{0\},\cdot)$ es un cuerpo de orden 3, por lo que es isomorfo a $\Z_3$.
\end{example}

\begin{exercise}
  Construya $\forall n \geq 1$, un cuerpo con $2^n$ elementos y $3^n$ elementos. Equivalentemente, encuentre en $\F_2[x]$ y $\F_3[x]$ un polinomio irreducible de grado $n$.
\end{exercise}

\begin{definition}
  Una extensión $K/F$ se dice \wea{algebraica} si todo $\a \in K$ es algebraico.
  Un cuerpo $K$ se dice \wea{algebraicamente cerrado} si todo $p \in K[x]$ admite una raíz en $K$.
\end{definition}

\begin{definition}
  El cuerpo de los números algebraicos es
  \[ \bar\Q = \{\a \in \C : \a \text{ es algebraico sobre } \Q\}. \]
\end{definition}

\begin{theorem}
  $\bar\Q$ es cuerpo y es algebraicamente cerrado.
\end{theorem}
...
\begin{lemma}
  Si $K/F$ y $[K:F] < \oo$, entonces $K/F$ es algebraica.
\end{lemma}

\begin{lemma}
  Si $F \leq K \leq L$ son extensiones finitas, entonces
  \[ [L:F] = [L:K][K:F]. \]
\end{lemma}

\begin{definition}
  $\a \in \C$ se dice \wea{entero algebraico} si existe $p \in \Z[x]$ mónico tal que $p(\a) = 0$.
\end{definition}

\begin{obs}
  El conjunto de los enteros algebraicos es un subanillo de $\bar\Q$.
\end{obs}

\subsection{Cuerpos de Orden Finito}

\begin{proposition}
  Si $K$ es cuerpo finito, entonces $\Char(K)$ es primo. Además, $\abs K = p^n$ para algún $n \geq 1$ y $p$ primo, y
  \[ (K,+) \simeq \br{\znz p}^n. \]
\end{proposition}

\begin{theorem}
  $\forall p$ primo y $\forall n \geq 2$ existe un cuerpo con $p^n$ elementos, y es único salvo isomorfismo.

  Equivalentemente, $\forall n \geq 2$ existe $f \in \F_p[x]$ irreducible con $\deg f = n$.
\end{theorem}

\begin{obs}
  $p^n \mod{(p^d-1)} = p^{n \mod{d}}$ para todo $n,d \geq 1$.
\end{obs}
...
\begin{lemma}
  Sea $\pi$ mónico irreducible en $\F_p[x]$ de grado $d$. Si $d \mid n$, entonces $\pi \mid x^{p^n}-x$.
\end{lemma}

\begin{obs}
  Sea $\pi \in \F_p[x]$ irreducible, $d = \deg\pi$ y $\pi \mid x^{p^n}-x$. Entonces $\pi \mid \gcd(x^{p^n}-x,x^{p^d}-x)$. Entonces $\pi \mid x^{\gcd(n,d)}-x$.
\end{obs}

\begin{lemma}
  $x^{p^n}-x$ es libre de cuadrados.
\end{lemma}

\begin{theorem}
  El producto de todos los polinomios mónicos irreducibles de grado $d \mid n$ es $\F_p[x]$ es $x^{p^n}-x$.
\end{theorem}

\begin{theorem}
  Sea $M_n(p)$ el número de polinomios irreducibles de grado $n$ en $\F_p[x]$ mónicos. Entonces
  \[ M_n(p) = \frac{1}{n} \sum_{d \mid n} \mu(d) p^{n/d} \]
  donde
  \[ \mu(d) = \begin{cases}
    1, &\text{si $d = 1$} \\
    (-1)^k, &\text{si $d$ es producto de $k$ primos distintos} \\
    0, &\text{si $d$ contiene cuadrados}
  \end{cases} \]
\end{theorem}

Para demostrar esto, se usa la fórmula de inversión de Möbius:

Si $g,f \colon \N \to \C$ son tales que
\[ g(n) = \sum_{d \mid n} f(d) \]
entonces 
\[ f(n) = \sum_{d \mid n} \mu(d) g\br{\frac{n}{d}}. \]

\fecha{Clase 26. Miércoles 17 de Junio}

\begin{definition}
  Sea $K/F$ una extensión de cuerpos. Decimos que es \wea{trascendente} si no es algebraica.
\end{definition}

\begin{example}
  $\C/\Q$ es una extensión trascendente.
\end{example}

\begin{theorem}
  Sea $\a \in \C$ algebraico sobre $\Q$ de grado $n$. $\exists C>0$ tal que
  \[ \abs{\a - \frac{p}{q}} > \frac{C}{q^n} \]
  para todo $p/q \in \Q$ con $q>0$ y $\gcd(p,q) = 1$.
\end{theorem}
\begin{proof}
  Supongamos que $\a \in \R$ es algebraico y $f = \min(\a,\Q)$. Por el teorema del valor medio, si tomamos $p/q \in \Q$ reducido con $p/q \neq \a$, entonces
  \[ - f \br{\frac pq} = f(\a) - f \br{\frac pq} = \br{\a - \frac pq} f'(\xi) \]
  para algún $\xi \in (p/q, \a)$.
  Supongamos que $\abs{\a - \frac pq} < 1$, entonces $\abs\xi \leq 1 + \abs\a$. Notemos que $\exists C(\a)>0$ tal que $\forall \zeta \in (p/q,\a)$,
  \[ \abs{f'(\zeta)} \leq C(\a). \]
  Luego
  \begin{align*}
      \abs{\a - \frac pq} \c \abs{f'(\xi)} &= \abs{f\br{\frac pq}} \\
      \implies \abs{\a - \frac pq} &> \frac{1}{C(\a)} \cdot \abs{f\br{\frac pq}} \\
      \implies \abs{\a - \frac pq} &\geq C \abs{f\br{\frac pq}} \geq \frac{C}{q^n}.
  \end{align*}
  Como $f$ es irreducible, entonces $f(p/q) \neq 0$. Luego $\abs{q^n f(p/q)} \geq 1$.
\end{proof}

\newpage
\begin{corollary}
  Sea $\ds L = \sum_{n\geq 0} 10^{-n!}$. Entonces $L$ es trascendente.
\end{corollary}
\begin{proof}
  Para $j \in \N$, defino $\ds p_j = 10^{j!} \sum_{n \leq j} 10^{-n!} \in \Z$. Definimos $q_j = 10^{j!} \in \Z$. Luego
  \begin{align*}
      \abs{L - \frac{p_j}{q_j}} &= \sum_{n \geq j+1} 10^{-n!} \\
      &< 10^{-(j+1)!} \br{1 + \frac{1}{10} + \frac{1}{100} + \dots + \frac{1}{10^n}} \\
      &= \frac{10}{9} \cdot 10^{-(j+1)!} \\
      &= \frac{10}{9} q^{-(j+1)} \\
      &< \frac{10}{9} q_j^{-j}. \quad \to\gets
  \end{align*}
\end{proof}
...
\end{theorem}
\begin{proof}
  Baker, \emph{Transcendental Number Theory}.
\end{proof}

\begin{theorem}
  [Lindemann--Weierstrass]
  Sean $\a_1, \dots, \a_n$ números algebraicos distintos, y sean $b_1, \dots, b_n$ algebraicos no nulos. Entonces
  \[ \sum_{i=1}^n b_i e^{\a_i} \neq 0. \]
\end{theorem}

\begin{definition}
  Sea $K/F$ una extensión. Decimos que $\gamma_1, \dots, \gamma_n \in K$ son \wea{algebraicamente independientes sobre $F$} si $\forall p \in F[x_1,\dots,x_n]$ no nulo se cumple que $p(\gamma_1,\dots,\gamma_n) \neq 0$.
\end{definition}

\begin{theorem}
  [Lindemann--Weierstrass v.2]
  Si $\a_1,\dots,\a_n$ son algebraicos y algebraicamente independientes sobre $\Q$, entonces $e^{\a_1}, \dots, e^{\a_n}$ son algebraicamente independientes.
\end{theorem}
...
\end{corollary}
\begin{proof}
  $e^{i\pi} + e^0 = 0$. Luego $i\pi$ no es algebraico, y entonces $\pi$ es trascendente.
\end{proof}

\begin{definition}
  $f \in K[x_1,\dots,x_n]$ se dice \wea{simétrico} si es invariante por permutación de variables. i.e.: si consideramos la acción $S_n \acts K[x_1,\dots,x_n]$ dada por
  \[ \sigma \cdot f(x_1,\dots,x_n) = f(x_{\sigma(1)},\dots, x_{\sigma(n)}) \]
  entonces $f$ es simétrico si y solo si $\sigma \cdot f = f$ para todo $\sigma \in S_n$.
\end{definition}

\begin{example}
  $x_1^2 + x_2^2, \quad x_1x_2x_3,\quad x_1x_2 + x_1 x_3 + x_2 x_3$.
\end{example}

\fecha{Clase 27. Lunes 22 de Junio}

Consideremos los siguientes problemas geométricos clásicos:
\begin{itemize}
  \item Construir un cubo con el doble del volumen de un cubo original.
  \item Dado un círculo, construir un cuadrado de misma área.
  \item Dado un ángulo, trisectarlo.
\end{itemize}

\begin{definition}
  Decimos que $(x,y) \in \R^2$ es \wea{dibujable} si a partir de $(0,0)$, $(1,0)$ y las reglas:
  \begin{enumerate}[(i)]
    \item $(0,0)$ y $(1,0)$ son dibujables,
    \item Dados 2 puntos, trazar una recta,
    \item Dados 2 puntos dibujables $p,q$, trazar el círculo de centro $p$ y radio $\norm{q-p}_2$,
    \item Usar los puntos de intersección de rectas y círculos trazados
  \end{enumerate}
  se puede obtener $(x,y)$.
\end{definition}

\begin{definition}
  Decimos que $z \in \R$ es \wea{constructible} si existe $p \in \R^2$ dibujable tal que $\norm{p}_2 = z$. Denotamos por $\Ccal$ el conjunto de los números constructibles.
\end{definition}

\begin{theorem}
  $\Ccal$ es subcuerpo de $\R$.
\end{theorem}
...
\begin{lemma}
  $\a \in \Ccal \implies \sqrt\a \in \Ccal$.
\end{lemma}

\begin{definition}
  Sea $K \subset \R$ cuerpo. Decimos que una recta $L \subset \R^2$ está \wea{definida en $K$} si contiene 2 puntos en $K^2$. Una circunferencia $C$ está \wea{definida en $K$} si su centro y radio pertenecen a $K^2$.
\end{definition}

\begin{proposition}
  Si $L$ es una recta definida sobre $K$, entonces $\exists a,b,c \in K$ tales que
  \[ L = \{(x,y) \in \R^2 : ax+by+c=0\}. \]
  Si $C$ es una circunferencia definida sobre $K$, entonces $\exists a,b,c \in K$ tales que
  \[ C = \{(x,y) \in \R^2 : x^2 + y^2 + ax + by + c = 0\}. \]
\end{proposition}

\begin{lemma}
  Sea $K \subset \Ccal$ subcuerpo. Si $\xi \in \Ccal$ se obtiene intersectando rectas y circunferencias sobre $K$, entonces existe $\a \in K$ tal que $\xi \in K[\sqrt\a]$.
\end{lemma}

\begin{theorem}
  $\a \in \R$ es constructible si y solo si existe una secuencia finita de cuerpos
  \[ \Q = F_0 \subset F_1 \subset F_2 \subset \dots \subset F_k \]
  tal que $\a \in F_k$ y $[F_j : F_{j-1}] = 2$.
\end{theorem}

\begin{corollary}
  Si $\a \in \Ccal$, entonces $\exists k$ tal que $[\Q(\a) : \Q] = 2^k$.
\end{corollary}
...
\end{proposition}
\begin{proof}
  Si $\sqrt\pi \in \Ccal$, entonces $[\Q(\pi) : \Q] < \oo$. \(\to\gets\)
\end{proof}

Para hablar de trisectar el ángulo, hablamos de ángulos constructibles.

\begin{definition}
  Decimos que $\a \in [0,2\pi]$ es un \wea{ángulo construcible} si $\exists r >0$ tal que $(r \cos\a, r \sin\a)$ es dibujable.
\end{definition}

\begin{obs}
  $\a$ es ángulo constructible si y solo si $\cos\a$ y $\sin\a$ son constructibles.
\end{obs}

\begin{proposition}
  $\pi/9$ no es constructible.
\end{proposition}
\begin{proof}
  Sea $\a = \cos(\pi/9)$. Entonces
  \[ \frac{1}{2} = \cos\br{\frac{\pi}{3}} = \cos\br{3 \c \frac{\pi}{9}} = 4 \cos^3\br{\frac{\pi}{9}} - 3 \cos\br{\frac{\pi}{9}}. \]
  Luego $\pi/3$ es raíz de $4x^3 - 3x - 1/2$, pero este polinomio es irreducible. Por lo tanto $\pi/9$ no es constructible.
\end{proof}

\fecha{Clase 28. Miércoles 24 de Junio}

\fecha{Clase 29. Miércoles 1 de Julio}

\begin{proposition}
  La clausura algebraica es única (salvo isomorfismo).
\end{proposition}
